...
void strings_completion_t_(int rc,
	...
	for(i=0;i<strings->count;i++){
		printf("child_path=%s\n",(strings->data)[i]);
		len = snprintf(buff,sizeof(buff),"%s/%s",root,(strings->data)[i]);
...
void watcher_fn_(zhandle_t *zh, int type, 
        int state, const char *path,void *watcherCtx){
	printf("type=%d;state=%d;path=%s;\n",type,state,path);
}
...
//        }
      ...
      zoo_add_auth(zh, line, ptr, ptr ? strlen(ptr) : 0, NULL, NULL);
    }else if (startsWith(line,"mytest")){
		fprintf(stderr, "mytest...\n");
		/*
		int node = 1;
		const char* ipport = "127.0.0.1:3340";
		int ipport_len = strlen(ipport);
		struct Stat stat;
		rc = zoo_exists(zh,root,0,&stat);
		if(ZNONODE == rc){
			rc = zoo_create(zh,root,NULL,-1,&ZOO_OPEN_ACL_UNSAFE,0,0,0);
			if(ZOK != rc){
				 fprintf(stderr, "Error %d \n", rc);
			}
		}
		...
		rc = zoo_aget_children(zh,root,1,strings_completion_t_,NULL);
		(void)ipport_len;
		(void)node;
		if(ZOK != rc){
			 fprintf(stderr, "Error %d \n", rc);
		...
		*/
		...
		int i;
		for(i = 0;i<10;i++){
			int len = snprintf(buff,sizeof(buff),"%s/%d",root,i);
			assert(len < sizeof(buff));
			rc = zoo_awexists(zh, buff, watcher, (void*) 0, my_stat_completion, strdup(buff));
			assert(ZOK == rc || ZNONODE == rc);
		}
		//int len = snprintf(buff,sizeof(buff),"%s/%d",root,node);
		//assert(len < sizeof(buff));		
		
		//rc = zoo_create(zh,buff,ipport,ipport_len,&ZOO_OPEN_ACL_UNSAFE,ZOO_EPHEMERAL,0,0);
		//if(ZOK != rc){
		//	 fprintf(stderr, "Error %d \n", rc);
		//}
